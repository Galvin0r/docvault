...
describe('GroupService', () => {
  ...
  it('getMembers merges and overrides defaults when params provided', () => {
    ...
    service.getMembers(9, { page: 2, size: 50, role: 'ADMIN' }).subscribe(r => expect(r).toEqual(resp));
    const req = http.expectOne(r =>
      r.url === '/api/groups/9/members' &&
      r.params.get('page') === '2' &&
  ...
  it('addMember -> POST /api/groups/:id/members with email param', () => {
    ...
    const req = http.expectOne(r =>
      r.url === '/api/groups/5/members' && r.params.get('email') === 'a@b.c'
    );
    expect(req.request.method).toBe('POST');
  ...
  it('changeRole -> PATCH /api/groups/:id/members/:userId/role with role param', () => {
    ...
    const req = http.expectOne(r =>
      r.url === '/api/groups/7/members/3/role' && r.params.get('role') === 'ADMIN'
    );
    expect(req.request.method).toBe('PATCH');
    expect(req.request.body).toBeNull();
    req.flush(null);
  ...
  it('getRequests -> GET /api/groups/:id/requests with params', () => {
    ...
    service.getRequests(8, { page: 1, size: 5, status: 'PENDING' }).subscribe(r => expect(r).toEqual(resp));
    const req = http.expectOne(r =>
...
describe('Group resolvers', () => {
  ...
  it('groupResolver returns Group', async () => {
    ...
    const obs = TestBed.runInInjectionContext(() => groupResolver(route, state)) as Observable<Group>;
  ...
  it('membershipResolver returns membership on 200 and null on error', async () => {
    ...
    const obs1 = TestBed.runInInjectionContext(() => membershipResolver(route, state)) as Observable<GroupMembership | null>;
    ...
    const obs2 = TestBed.runInInjectionContext(() => membershipResolver(route, state)) as Observable<GroupMembership | null>;
  ...
  it('joinRequestResolver returns GroupJoinRequest or null', async () => {
    ...
    const obs1 = TestBed.runInInjectionContext(() => joinRequestResolver(route, state)) as Observable<GroupJoinRequest | null>;
    const p1 = firstValueFrom(obs1);
    ...
    const obs2 = TestBed.runInInjectionContext(() => joinRequestResolver(route, state)) as Observable<GroupJoinRequest | null>;
    const p2 = firstValueFrom(obs2);
